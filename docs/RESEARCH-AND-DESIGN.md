...
Manrope is the single UI/body and heading family. Headings are bold (700) for clarity; the short human phrase that closes a headline is set in the same face and weight, distinguished only by the clay (or peach/moss on dark) accent color. An earlier italic serif accent was removed in favor of clearer, more direct type. The site loads Manrope through Next's font integration, not third-party font stylesheets on every visit. No font files are included in the ZIP. Changing typography later should happen through the font declaration and the shared CSS tokens, not per-page overrides.
